 ...
 *----------------------------------------------------------------------------------*/	
   ...
   ProdList populateList = populateProdList();
//   printf("Hello");
...
{ 
   int count;    //No. of chocolate records in data
	product data[] = { 	{"1701", {"Toblerone", 135}, 150.75, 20}, {"1356", {"Ferrero", 200}, 250.75, 85},
						...
						{"1170", {"Meiji", 75}, 75.50, 60}, {"1310", {"Nestle", 100}, 124.50, 70},
						{"1807", {"Valor", 120}, 149.50, 90}, {"1455", {"Tango", 75}, 49.50, 100},
						{"1703", {"Toblerone", 100}, 125.75, 60}, {"1689", {"Lindt", 100}, 250.75, 15},
	...
	for(x = 0; x < count; x++){
		...
		if (temp != NULL){
			temp->prod = data[x]; temp->plink = NULL; *ptr = temp; ptr = &temp->plink;  
		}
	}
   return  L;
}
...
//	ProdList trav;
	...
	for(; L != NULL; L = L->plink, num++){
		
	// You may use this as part of your code, just complete the statement, then uncomment
		printf("\n%-7s", L->prod.prodID);         //Complete this to display the ID of a choco record
		printf("%-12s", L->prod.prodDesc.name);            //Complete this to display the name of a choco record
		printf("%-15d", L->prod.prodDesc.weight);            // Complete this to dispaly the weight of a choco record
		
	}
	
 
	printf("\n\nNo. of elements :: %d", num);   //Complete this line and uncomment
...
//   ProdList *trav;
   ...
   for(; (*L) != NULL && strcmp((*L)->prod.prodID, ID); (*L) = (*L)->plink);
   
   temp = (ProdList)malloc(sizeof(ProdList));
   temp = *L;
   *L = (*L)->plink;
   free(temp);
